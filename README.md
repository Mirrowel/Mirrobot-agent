...
**Latency**: the default entry path is the external **mention-worker** (below) — Actions run only when something actually happened. The worker polls every **30 seconds** via a self-rescheduling Durable Object alarm (cron triggers are broken on our account — registered but never dispatched; see the worker README; the backup-cron option is documented in its `wrangler.toml`, not attached). A manual `workflow_dispatch` run polls on demand anytime; an in-repo fallback schedule is a documented opt-in (see the workflow header) — mark-read acks make all paths mutually exclusive.
