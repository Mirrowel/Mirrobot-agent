...
Parsed by the shared script `.github/scripts/route-comment.sh` (the same script the workflows re-run for validation, so routing semantics cannot drift between copies). The matrix is **derived from trigger stems**: `bot-config.sh` resolves `BOT_TRIGGERS` (variable → identity-derived → the stock mirrobot words) into `BOT_TRIGGER_STEMS`, and every stem matches:
