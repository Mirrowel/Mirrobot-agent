...
Behavior changes carry their pin changes in the same commit. A red suite after your edit means the suite disagrees with your change; that is the suite doing its job. Either your change is wrong, or the pin needs updating *because you decided the behavior should differ*. Both are fine; silently deleting a pin is not.
...
The suite tests itself too: its own CI run fires on every `.github/**` push, including changes to this suite, so a broken checker gets caught by itself.
