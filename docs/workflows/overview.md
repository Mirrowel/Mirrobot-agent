...
**Base-branch sync doctrine** (for the two workflows above that run from a PR's base branch): platform changes land on main; the integration branch ("dev" here, whatever yours is named) needs no per-batch sync; when it *should* be current (the stub or gate changed), sync with `git merge main` into it, never copy-commits. Full story: [design.md](../design.md#the-update-doctrine).
