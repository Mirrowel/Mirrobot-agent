...
- **Auto-cards ("field notes")** — the page deals eight random cards from `excerpts.json`, regenerated on every deploy by the [Excerpts Refresh](workflows/excerpts-refresh.md) workflow from your repos × your identities, hidden content excluded, quality-gated (old-era conversational acks and pure one-liners filtered out). Recency is tiered: posts newer than `EXCERPT_DAYS` (180) fill the pool first and older material only tops up below the `EXCERPT_MIN` (40) floor. The site publishes from that workflow's Pages artifact (build type: workflow), so no commits to `main` are involved. Run the harvester locally with `EXCERPT_REPOS`/`EXCERPT_BOTS`/`EXCERPT_MAX`/`EXCERPT_DAYS`/`EXCERPT_MIN` env overrides; the JSON is static and same-origin, so visitors spend no API budget on it.
