...
- **Auto-cards ("field notes")** — the page deals up to eight random cards from `excerpts.json`, drawn **only from items newer than the harvest's recency window** (`days` rides in the JSON; `EXCERPT_DAYS`, default 180) — older material may sit in the pool as backfill but never renders. Empty recent tier: the section degrades to a notice that jokes about the silence and suggests summoning the bot to investigate. Regenerated on every deploy by the [Excerpts Refresh](workflows/excerpts-refresh.md) workflow from your repos × your identities, hidden content excluded, quality-gated (old-era conversational acks and pure one-liners filtered out). The site publishes from that workflow's Pages artifact (build type: workflow), so no commits to `main` are involved. Run the harvester locally with `EXCERPT_REPOS`/`EXCERPT_BOTS`/`EXCERPT_MAX`/`EXCERPT_DAYS`/`EXCERPT_MIN` env overrides; the JSON is static and same-origin, so visitors spend no API budget on it.
- **Badge link parameters** — two knobs ride on the URL the badge wraps:
  - `?bot=YOUR-BOT-NAME` — your deployment's identity, so the page tells visitors who to mention back home. If the deployment answers to more than one name (account plus `[bot]` app form), comma-separate them: `?bot=MyBot,MyBot[bot]` — the page's arrival chip then lists all of them.
  - `&repo=OWNER/REPO` — repoints the page's live-activity strip at the linking repository, so a visitor from a foreign repo sees that repo's recent agent activity instead of the home repos'.
  - Example: `[![Ask Mirrobot](https://your-pages-host/badge.svg)](https://your-pages-host/?bot=MyBot,MyBot[bot]&repo=You/YourRepo)`
