...
3. While you're there: require a pull request before merging (the agent's reviews read better as suggestions on PRs), and **do not** add a "Restrict updates" rule with admin-only bypass; it deadlocks bot merges entirely.
...
|---|---|---|
| Compliance watch-list | `FILE_GROUPS_JSON` in `compliance-check.yml` | Which file groups the final audit checks, this is repo-specific. See [customization.md](customization.md#the-compliance-watch-list) |
...
- [security.md](security.md), the threat model this platform is built against (read it before you open it to strangers)
