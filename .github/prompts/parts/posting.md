...
```
When the reply anchor rendered into the command above is empty (you were summoned by a new discussion's body, or the trigger comment could not be pinned), post top-level with the same mutation but `-F r=null` in place of the anchor argument (the variable stays declared and used; null means no reply anchor). A top-level post is also legitimate when your answer genuinely serves the whole thread rather than the asker — use your judgment, and when you deviate from the reply default, say why in one line.
