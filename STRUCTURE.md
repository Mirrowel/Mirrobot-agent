...
**Tests:** `.github/scripts/scrub-fixtures.sh` (342 security fixtures covering the scrub, roster transforms, router decision matrix, mention/discussion pipelines, and the bash/read/edit/write permission deny matrices) and `.github/scripts/prompt-rule-fixtures.sh` (393 pinned prompt rules), wired to CI via `.github/workflows/scrub-fixtures.yml`.
